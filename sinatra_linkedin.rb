...
configure do
  ...
  set :api, "a9rhujr4dmjw"
  set :secret, "YiXTYbAxN8w9fa17"
end
...
get "/auth" do
  client = LinkedIn::Client.new(settings.api, settings.secret)
  request_token = client.request_token({:oauth_callback => "http://#{request.host}:#{request.port}/auth/callback"}, :scope => 'r_network')
...
get "/auth/callback" do
  client = LinkedIn::Client.new(settings.api, settings.secret, {:scope => 'r_fullprofile r_emailaddress r_network'})
